...
type Props = { frame: LiveFrame | null; status: LiveStatus; paused: boolean; send: (message: object) => void };

export function LiveTraining({ frame, status, paused, send }: Props) {
  ...
  const stimulate = (sign: number) => {
    if (enabled && frame) send({ feedback: sign * strength, fly: allFlies ? null : frame.selected, move: frame.move });
  };
  ...
  return <>
    <div><strong>LIVE LEARNING</strong>
      ...
      <div className="controls">
        <button disabled={!canTrain} onClick={() => send({ learning: 'pretrained' })} title="Replace the live session with a trainable copy of the trained readout">Retrain existing readout</button>
        <button disabled={!canTrain} onClick={() => send({ learning: 'reset' })} title="Replace the live session with a blank readout">Start blank</button>
      </div>
    ...
    <div className="training-feedback"><strong>POSITIVE / NEGATIVE STIMULI</strong>
      ...
      <p className="feedback-result" role="status" aria-live="polite">{result}</p>
