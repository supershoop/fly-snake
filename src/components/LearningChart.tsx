...
const PAD = { left: 38, right: 118, top: 14, bottom: 28 };
...
export function LearningChart({ runs, current }: { runs: LearningRun[]; current: keyof typeof WIRING }) {
  ...
  if (!hasSeries) return <p className="chart-empty">The learning curve appears after the first two finished games.</p>;
  ...
  return <figure className="learning-chart">
    <figcaption>Food per game while learning: rolling average of the last {WINDOW} finished games, all flies · same learning rule for every line</figcaption>
    ...
    <div className="learning-chart-plot" ref={plotRef}>
