...
export function Environment({ frame, status, paused, pending, onSelect, onHuman, onResume }: {
  frame: LiveFrame | null; status: LiveStatus; paused: boolean; pending: PendingCommand | null; onSelect: (fly: number) => void; onHuman: (direction: string) => void; onResume: () => void;
}) {
  ...
  return <div className="environment">
    ...
    <div className={`board-stage ${many ? 'swarm-stage' : ''}`}>
      ...
      {(paused || frame.manual || pending) && <div className={`game-overlay ${pending ? 'is-pending' : ''}`}><span>{pending ? <><i className="spinner"/>{pending.message}<small>Updating the simulation…</small></> : paused ? <>Simulation paused<small>Resume to continue the game</small></> : <>Sensory override<small>Release the input to continue</small></>}</span></div>}
    </div>
