...
import { EventStimuli } from './components/EventStimuli';
import { BrainLearning } from './components/BrainLearning';
...
export function App() {
  ...
  return <>
    ...
    <main id="experiment">
      ...
      <BrainLearning frame={frame} status={status} send={send}/>
      ...
      {frame?.policy !== 'learning' && <details className="lab-disclosure event-stimuli-standalone" open><summary><span className="lab-icon"><Icon name="sliders" size={21}/></span><span className="disclosure-title">Game-event stimuli<small>Choose positive, negative or no stimulus for food and death.</small></span><span className="disclosure-chevron" aria-hidden="true">+</span></summary><div className="model-status"><EventStimuli frame={frame} status={status} paused={paused} send={send}/></div></details>}
