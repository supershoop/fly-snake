...
export function App() {
  ...
  return <>
    ...
    <main id="experiment">
      ...
      <ExperimentControls frame={frame} status={status} paused={paused} send={send}/>
      ...
      <div className="workbench">
        <section className="panel environment-panel" aria-labelledby="environment-title">
          ...
          <Environment frame={frame} status={status} paused={paused} picked={picked} onPick={pick} onSelect={select => send({ select })} onHuman={human => send({ human })} onResume={resume}/>
        ...
        <section className="panel brain-panel" aria-labelledby="brain-title">
          ...
          {atlas ? <BrainScene atlas={atlas} frame={activity} silenced={silenced}/> : <p className="loading" role="status">{error ? 'Brain atlas unavailable' : 'Loading measured anatomy…'}</p>}
        ...
        <section className="panel fly-panel" aria-labelledby="body-title">
          ...
          <FlyScene/>
          <div className="body-caption"><em>Drosophila melanogaster</em><span>Anatomical reference<br/>No motor simulation</span></div>
          <div className="panel-bottom"><span>Flybody surface mesh</span><span>Drag to rotate</span></div>
        </section>
      </div>
      {frame && (picked.length > 0 || frame.policy === 'learning') && <section className="experiment-lab" id="lab" aria-label="Experiment lab">
        {picked.length > 0 && <details className="lab-disclosure" open><summary><span className="lab-icon"><Icon name="brain" size={21}/></span><span className="disclosure-title">Lesion lab<small>Silence a circuit in the picked {picked.length === 1 ? 'fly' : 'flies'}. Observe what changes.</small></span><span className="disclosure-tag">{picked.length === 1 ? `Fly ${picked[0] + 1}` : `${picked.length} flies`}</span><span className="disclosure-chevron" aria-hidden="true">+</span></summary><LesionLab frame={frame} status={status} paused={paused} types={types} picked={picked} send={send}/></details>}
        {frame.policy === 'learning' && <details className="lab-disclosure" open><summary><span className="lab-icon"><Icon name="sliders" size={21}/></span><span className="disclosure-title">Live learning<small>Shape the readout with reward and punishment.</small></span><span className="disclosure-tag">Learning active · {frame.learning.games} games · mean {average}</span><span className="disclosure-chevron" aria-hidden="true">+</span></summary><div className="model-status readout learning"><LiveTraining frame={frame} status={status} paused={paused} send={send}/></div></details>}
      </section>}
